<template>
    ...
    <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap">
      <div>
        ...
        <p class="text-secondary mb-0" style="font-size: 0.875rem; max-width: 320px;">Descubre las innovaciones que están causando sensación en este momento</p>
      </div>
      ...
      <div class="col-12 col-sm-6 col-lg-4" v-for="project in projects" :key="project.id" @click="router.push(`/projects/${project.id}`)">
        <div class="card border border-2 rounded shadow-sm h-100">
          <div class="position-relative">
            <img v-if="project.project_images.length" :src="`${project.project_images[0].image}`" :alt="`Imagen del proyecto ${project.name}`" class="card-img-top" style="height: 16rem; object-fit: cover;" />
